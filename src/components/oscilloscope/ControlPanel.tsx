...
import type { WaveformSettings, TimebaseSettings } from '@/hooks/useWaveformGenerator';
  ...
  return (
    <div className="bg-card border border-border rounded-lg p-4 space-y-6">
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          <Label className="text-xs text-muted-foreground">
            Level: {triggerLevel.toFixed(2)} V
          </Label>
          <Slider
            value={[triggerLevel]}
            onValueChange={([value]) => onTriggerLevelChange(value)}
            min={-5}
            max={5}
            step={0.1}
          />
      ...
      <div className="space-y-3">
        <h3 className="text-sm font-semibold text-primary uppercase tracking-wide">Signal Gen</h3>
